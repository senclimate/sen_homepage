  ...
  let $quickSearch = $('.filter-search').keyup(
    ...
    $('.pub-filters').on('change', function () {
      ...
      if ($this.attr('type') === 'checkbox') {
        ...
        let selected = $('input[data-filter-group="' + filterGroup + '"]:checked')
          .map(function () {
            return this.value;
          })
          .get();
        ...
        if (selected.length) {
          pubFilters[filterGroup] = selected.join('');
        } else {
          delete pubFilters[filterGroup];  // No selection: clear the filter
        }
      } else {
        // For dropdowns
        pubFilters[filterGroup] = $this.val();
      }
      ...
      if (filterGroup === 'pubtype') {
        let url = $this.val();
        if (url.startsWith('.pubtype-')) {
          window.location.hash = url.substring(9);
        } else {
